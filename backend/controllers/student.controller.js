...
<body>
    ...
    res
      .status(201)
      .json({ message: "Student registered successfully", student, token });
  } catch (error) {
...
module.exports.verifyStudentDocument = async (req, res) => {
  try {
    ...
    if (!student.verified) {
      return res
        .status(400)
        .json({ message: "Student email has not been verified." });
    }

    if (!student.feeReceipt) {
      return res
        .status(400)
        .json({ message: "Fee receipt has not been uploaded by the student." });
    }
    ...
    if (!student.trainingLetter) {
      return res
        .status(400)
        .json({
          message: "Training letter has not been uploaded by the student.",
        });
    }
...
module.exports.sendErrorEmail = async (req, res) => {
  ...
  try {
    ...
    if (fileType === "feeReceipt" && student.feeReceipt) {
      ...
      return res
        .status(400)
        .json({ message: "Invalid file type or no file found to remove" });
    }
    ...
    if (fileRemoved && fs.existsSync(filePath)) {
      fs.unlink(filePath, (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ message: "Error deleting the file from disk" });
        }
    ...
    await sendEmail(
      student.email,
      `Your ${fileType} Has Been Rejected`,
      `
    <html>
    <head>
        <style>
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <p class="message"><b>Reason:</b>${req.body.reason}:${
        req.body.customMessage
                ...
                ${
                  req.body.contactPerson
                    ? `<p class="message">Contact Person: ${req.body.contactPerson}</p>`
                    : ""
                }
                <p class="footer">If you believe this is a mistake, please contact the administration.</p>
            </div>
...
`
    ...
    res
      ...
      .json({
        message:
          "Password reset link sent to your email.Please Check Spam Folder too",
      });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
module.exports.getFeedbackFormAbet = async (req, res) => {
  try {
    const { studentId } = req.params;

    const questions = await AbetQuestionStudent.find();
    console.log(questions);
    const feedback = await FeedbackStudentAbet.findOne({
      student: studentId,
    ...
    res.status(200).json({
      ...
      postGraduationPlan: feedback?.postGraduationPlan || "",
      postGraduationPlanDetails: feedback?.postGraduationPlanDetails || "",
    });
  } catch (err) {
...
module.exports.submitFeedbackFormAbet = async (req, res) => {
  try {
    const { studentId } = req.params;
    const { levels, postGraduationPlan, postGraduationPlanDetails } = req.body;
    ...
    const responses = Object.entries(levels).map(([questionId, level]) => ({
        question: questionId,
        levelOfAttainment: level,
    }))

    console.log(responses);

    const feedback = await FeedbackStudentAbet.findOneAndUpdate(
        {student: studentId},
        {
            student: studentId,
            responses,
